...
public class EnemyAI : MonoBehaviour
{
    ...
    {
        if (isPlayerCaught)
            return; // 👈 Полный стоп логики пока не ресетнут
    ...
    private void StopAndAttack()
    {
        ...
        if (!alreadyAttacked)
        {
            ...
            RespawnManager.Instance.TriggerRespawnBarry(player.gameObject, gameObject);
        ...
        if (isPlayerCaught) return; // ← уже поймал, ничего не делаем

        isPlayerCaught = true; // ← флаг, что игрок пойман
    }
    public void SpeakReset()
    {
        StartCoroutine(CallWithDelay(delay));
    }
